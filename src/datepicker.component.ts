            ...
            <div 
              *ngFor="let day of calendarDays;let i=index;"
              ...
              [ngStyle]="{'cursor': day == 0 ? 'initial' : 'pointer',
                          ...
                          'color': isHoveredDay(day) ? colors['black'] : getDayFontColor(day),
                          'pointer-events': !isDisableDay(day) ? 'none' : ''
                          }"
...
export class DatepickerComponent implements OnInit, OnChanges {
  ...
  @Input() rangeEnd: Date;
  @Input() datesToDisable: Array<String>;
  // data
  ...
  selectedDate: string;
  selectedDateIsChosen: string;


  constructor(private renderer: Renderer, private elementRef: ElementRef, private datePipe: DatePipe) {
  ...
  onSelectDay(day: Date): void {
    //this.selDates = [];
    this.selectedDate = this.datePipe.transform(day, 'MM/dd/yyyy');
    if (this.selDates.length == 0) {
      this.date = day;
    }
    if (this.selectedDate) {
      if (this.selDates.indexOf(this.selectedDate) !== -1) {
        this.selDates.splice(this.selDates.indexOf(this.selectedDate), 1);
      } else {
        this.selDates.push(this.selectedDate);
      }
    }
  ...
  isChosenDay(day: Date): boolean {
    this.selectedDateIsChosen = this.datePipe.transform(day, 'MM/dd/yyyy');
    if (day && this.selDates.length === 0) {
      if (this.selDates.indexOf(this.selectedDateIsChosen) !== -1) {
    ...
    else if (day && this.selDates.length !== 0) {
      if (this.selDates.indexOf(this.selectedDateIsChosen) !== -1) {
        this.isChangeColor = true;
      } else {
        this.isChangeColor = false;
